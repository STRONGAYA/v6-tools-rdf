...
from vantage6_strongaya_rdf.data_processing import add_missing_data_info, extract_subclass_info
...
class TestRDFStoreSetup:
    ...
    def test_rdf_store_data_loading(self, rdf_store):
        ...
        assert len(bindings) > 0, "No data found in RDF-store - test data may not have loaded properly"
...
class TestRDFAlgorithmIntegration:
    ...
    def test_collect_sparql_data_with_rdf_store(self, rdf_store):
        ...
        try:
            ...
            endpoint = rdf_store["endpoint"]
            variables_to_describe = ["ncit:C28421"]  # Use a variable that should exist in test data
            ...
            try:
                ...
                print(f"collect_sparql_data returned DataFrame with shape: {result.shape}")
